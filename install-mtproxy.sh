...
printf '%sn' "${SECRETS[@]}" > "${CONF_DIR}/user-secret"
...
ExecStart=/bin/sh -lc '/usr/local/bin/mtproto-proxy -u mtproxy -p 8888 -H "$PORT" -S "$(sed 's/^/-S /' /etc/mtproxy/user-secret)" --aes-pwd /etc/mtproxy/proxy-secret /etc/mtproxy/proxy-multi.conf -M "$WORKERS"'
Restart=on-failure
RestartSec=3
LimitNOFILE=65535
